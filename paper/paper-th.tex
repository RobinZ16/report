...
\begin{proposition}
  ...
  corresponding 4-cycle $\set{z_0, z_1, z_2, z_3}$ of $\phi_c(z)$ can
  be parametrized over $\C$ by
  \begin{equation}
    \label{eq:4-param}
    \begin{gathered}
      c = \frac{1 - 4t^3 - t^6}{4t^2(t^2 - 1)}, \\
      z_0 = \frac{t^4 - t^2 + \sqrt{(t^4 - 1)(t^2 + 2t - 1)}}{2t(t^2 -
        1)},\,
      z_1 = \frac{1 - t^2 + t \sqrt{(t^4 - 1)(t^2 + 2t - 1)}}{4t^2(t^2
        - 1)}, \\
      z_2 = \frac{t^4 - t^2 - \sqrt{(t^4 - 1)(t^2 + 2t - 1)}}{2t(t^2 -
        1)},\,
      z_3 = \frac{1 - t^2 - t \sqrt{(t^4 - 1)(t^2 + 2t - 1)}}{4t^2(t^2
        - 1)},
    \end{gathered}
  \end{equation}
  where $t = z_0 + z_2$.
...
\begin{lemma}[Panraksa, Theorem~1.5.1 in \cite{MR2982105}]
  \label{lem:z0+z2}
  Let $c \in \Q$, and let $\set{z_0, z_1, z_2, z_3} \subset K$ be a
  4-cycle of $\phi_c(z) = z^2 + c$, where $K$ is a quadratic extension
  to $\Q$. Then $t = z_0 + z_2$ is rational.
\end{lemma}
...
\begin{theorem} [Panraksa, Theorem~1.5.2 in \cite{MR2982105}]
  \label{th:all}
  All combinations $(c, \set{z_0, z_1, z_2, z_3})$, where $c \in \Q$
  and $\set{z_0, z_1, z_2, z_3} \subset K$ is a corresponding 4-cycle
...
showed that for a 4-cycle $\set{z_0, z_1, z_2, z_3} \subset \U$, $z_0
+ z_2$ must be rational (i.e., $z_0$ and $z_2$ must be of the form $a
\pm b \sqrt{d}$ for some $a, b, d \in \Q$), and similarly for $z_1$
and $z_3$. Therefore, $(z - z_0)(z - z_2) \in \Q[z]$ and $(z - z_1)(z
- z_3) \in \Q[z]$, and hence each 4-cycle in $\U$ shows up as two
...
consider $x \in \U \setminus \Q$, in which case
\[
...
\begin{case}
  ...
  case. See the attached Mathematica package \texttt{mma/abc.m} for
  details.
\end{case}
...
\begin{case}
  ...
  this polynomial (see \texttt{mma/abc.m}), and when we plug in the
  ...
  \texttt{mma/abc.m}). It has degree $8$ in $a$ and degree $9$ in
...
is to prove Conjecture~\ref{cj:galois-conjugate-re} (of a completely
...
crucial to proving Lemma~\ref{lem:z0+z2}, which was later used to
...
\begin{lemma} [Panraksa, Lemma~2.3.1 in \cite{MR2982105}]
  ...
  \begin{enumerate}[(i)]
  \item $z_2 = \ol{z_0}$, where $\ol{z_0}$ denotes the Galois
    conjugate of $z_0$ in $K$;

  \item $\{z_0, z_1, z_2, z_3\} \cap \{\ol{z_0}, \ol{z_1}, \ol{z_2},
    \ol{z_3}\} =\es.$
  \end{enumerate}
\end{lemma}

Panraksa then proves that the second case $\{z_0, z_1, z_2, z_3\} \cap
\{\ol{z_0}, \ol{z_1}, \ol{z_2}, \ol{z_3}\} = \es$ is impossible, from
which Lemma~\ref{lem:z0+z2} immediately follows. We managed to prove a
...
\begin{proof}
  ...
  that $\t \equiv \phi_c^k$ on the entire cycle $\set{z_0, \dots,
    z_{N-1}}$.
...
$z_0$ is rational. Consequently the entire cycle lies in $\Q$, and we
can reduce the problem of finding quadratic $N$-cycles to purely
finding rational $N$-cycles. In particular, for $N = 5$, we already
know there are no rational 5-cycles, so Conjecture~\ref{cj:n=5-zero},
the main conjecture of Subsection~\ref{subsec:quadratic-5} that says
no quadratic 5-cycles exist for $c \in \Q$, follows automatically. We
record this corollary as follows.
...
\ol{z_0}$, in which case the entire cycle lies in $\Q$; or $z_0 =
\ol{z_{\frac{N}{2}}}$, in which case $z_0 + z_{\frac{N}{2}}$ is
rational, and consequently the trace $z_0 + \phi_c(z_0) + \cdots +
\phi_c^{N-1}(z_0)$ is rational. Either case, the point on $C_0(N)$
...
$C_0(6)$), we can easily show by exhaustion that the only quadratic
6-cycle is defined over $\Q(\sqrt{33})$, with $c = -\frac{71}{48}$ and
\begin{equation}
  \label{eq:6-cycle}
...
characteristics, then some Chebotarev density-type argument might
yield the full conjecture over number fields (which have
characteristic zero).
