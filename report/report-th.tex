...
singularities.\footnote{%
  We know five regular rational points on $C_P$: $(0, 0)$,
  $(4, \frac{1}{3})$, $(1, \frac{8}{3})$, $(6, 9)$, and a point at
  infinity $[0, 1, 0]$; and three singular rational points on $C_P$
  that turns out to be nodes: $(3, 0)$, and two points
  at infinity --- $[1, 3, 0]$ and $[1, 0, 0]$. For each node there are
  at most two rational points ``upstairs'' in the normalization, so we
  expect the normalization of $C_P$ to have at most 11 rational
  points. See \texttt{mma/pab-ratpoint.nb} for details.}
